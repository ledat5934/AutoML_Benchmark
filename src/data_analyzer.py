...
class DataAnalyzer:
    ...
    def build_eda_report(self, project: ProjectInfo) -> str:
        ...
        if train_files:
            ...
            df = pd.read_csv(train_path)  # load full dataset for comprehensive analysis
    ...
    def generate_dataset_json(
        ...
        # Attempt to locate a primary *train* CSV for profiling
        train_csvs = [p for p in candidate_paths if p.is_file() and "train" in p.name.lower() and p.suffix.lower() in {".csv", ".parquet"}]
        profile_df: Optional["pd.DataFrame"] = None
        if train_csvs:
            train_path = train_csvs[0]
            logger.info("Building ydata_profiling report from %s (full dataset)", train_path)
            if train_path.suffix.lower() == ".csv":
                profile_df = pd.read_csv(train_path)
            else:
                profile_df = pd.read_parquet(train_path)
            
            logger.info("Loaded dataset with shape: %s", profile_df.shape)
        else:
            logger.warning("No train CSV/Parquet found for profiling in project '%s'", project.name)
